...
## ⚠️ 最初に: あなたは認証情報を扱いません

**note の Cookie をあなたが受け取ることも、設定することもありません。**

値はオーナーが **GitHub Secrets に自分の手で登録済み**です。投稿は GitHub Actions の
ランナーの中で起き、`os.getenv()` で読まれます。あなたが見るのは Secret 名だけです。

```
NOTE_SESSION_V5       ← オーナーが登録済み。値は誰も（Claude も）見ない
NOTE_GQL_AUTH_TOKEN   ← 同上
```

だから次のようになります。

- **Cookie の値をチャットで要求しない**（`CLAUDE.md` の運用原則）
- **貼られても使わない。** 値の変更（note を再ログイン）を促す
- **あなたのローカル環境に設定する必要もない。** このリポジトリの作業に
  note への接続は不要で、テストは `requests` をモックしている

**もし「認証情報は扱えません」と判断しかけたら、それは正しい判断ですが、
このタスクには当てはまりません。** あなたに渡されるものが無いからです。
コードとテストだけで作業が完結し、実際の投稿は Actions が行います。

過去に一度、別セッションで「Cookie を渡す／渡さない」の押し問答になりました。
**渡す必要が最初から無かった**というのが結論です。
