...
オーナーの note ログイン情報を預かることになり、note 側の仕様変更で壊れる。
上の「認証情報はチャットに要求しない」に反するので採用しない。

はてなブログ AtomPub は公式 API で、認証はパスワードではなく **API キー**
（WSSE）。オーナーは Secret 名を登録するだけでよい。実装は
`src/publishers/hatena.py`、必要な Secret は `HATENA_USER_ID` /
`HATENA_BLOG_ID` / `HATENA_API_KEY`。

投稿は既定で**下書き**（`publishing.hatena_draft`）。承認ゲートを人が通した
後とはいえ、新しい外部連携が初回から公開すると書式崩れに気づく前に世に出る。
1本目を確認したら false にすれば、以後は `/approve` だけで公開される。
...
**点検は異常が無くても自分の記録を残す。** `oplog --workflow morning-check` で
`runs.jsonl` に1行書いて push する。2026-08-24 と 08-25 の点検は発火したのに
痕跡がゼロで、動いたのか黙って落ちたのか判別できなかった。**黙って失敗する
見張り番は、静かな夜と区別できない。**

`opreport` は**ワークフローごとの最新の記録だけ**を見る。直した失敗を毎朝
蒸し返すと、点検が解決済みの問題を調べ直すことになる。時間窓で切らないのは、
点検が1日飛んでも壊れたままなら報告してほしいため。
